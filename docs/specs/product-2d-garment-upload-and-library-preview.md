...
開発タブで調整した **袖丈・着丈指定・グレーディング情報を含むカスタム服データ（リグ付き SVG 由来）** と **商品画像** をまとめてサーバーへ送り、`products` に永続化する。商品データベース画面では **画像グリッド＋商品情報** で一覧し、カード操作で **モデル＋登録したリグ付き SVG を同一フィットパイプラインでプレビュー** できるようにする。
