...
DROP POLICY IF EXISTS "Users can view assets of their shop" ON assets;
DROP POLICY IF EXISTS "Users can create assets in their shop" ON assets;
DROP POLICY IF EXISTS "Users can update assets of their shop" ON assets;
DROP POLICY IF EXISTS "Users can delete assets of their shop" ON assets;
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'assets' AND column_name = 'shop_id' AND data_type = 'text'
  ) THEN
    EXECUTE format('CREATE POLICY "Users can view assets of their shop"
      ON assets FOR SELECT
      USING (shop_id = current_shop_id()::text)');
    EXECUTE format('CREATE POLICY "Users can create assets in their shop"
      ON assets FOR INSERT
      WITH CHECK (shop_id = current_shop_id()::text)');
    EXECUTE format('CREATE POLICY "Users can update assets of their shop"
      ON assets FOR UPDATE
      USING (shop_id = current_shop_id()::text)');
    EXECUTE format('CREATE POLICY "Users can delete assets of their shop"
      ON assets FOR DELETE
      USING (shop_id = current_shop_id()::text)');
  -- shop_idがUUID型の場合
  ELSIF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'assets' AND column_name = 'shop_id' AND data_type = 'uuid'
  ) THEN
    EXECUTE format('CREATE POLICY "Users can view assets of their shop"
      ON assets FOR SELECT
      USING (shop_id = current_shop_id())');
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'conversations' AND column_name = 'shop_id' AND data_type = 'text'
  ) THEN
    EXECUTE format('CREATE POLICY "Users can view conversations of their shop"
      ON conversations FOR SELECT
      USING (shop_id = current_shop_id()::text)');
    EXECUTE format('CREATE POLICY "Users can create conversations in their shop"
      ON conversations FOR INSERT
      WITH CHECK (shop_id = current_shop_id()::text)');
    EXECUTE format('CREATE POLICY "Users can update conversations of their shop"
      ON conversations FOR UPDATE
      USING (shop_id = current_shop_id()::text)');
  -- shop_idがUUID型の場合
  ELSIF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'conversations' AND column_name = 'shop_id' AND data_type = 'uuid'
  ) THEN
    EXECUTE format('CREATE POLICY "Users can view conversations of their shop"
      ON conversations FOR SELECT
      USING (shop_id = current_shop_id())');
...
DROP POLICY IF EXISTS "Users can view messages of their shop" ON messages;
DROP POLICY IF EXISTS "Users can create messages in their shop" ON messages;
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'messages' AND column_name = 'shop_id' AND data_type = 'text'
  ) THEN
    EXECUTE format('CREATE POLICY "Users can view messages of their shop"
      ON messages FOR SELECT
      USING (shop_id = current_shop_id()::text)');
    EXECUTE format('CREATE POLICY "Users can create messages in their shop"
      ON messages FOR INSERT
      WITH CHECK (shop_id = current_shop_id()::text)');
  -- shop_idがUUID型の場合
  ELSIF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'messages' AND column_name = 'shop_id' AND data_type = 'uuid'
  ) THEN
    EXECUTE format('CREATE POLICY "Users can view messages of their shop"
      ON messages FOR SELECT
      USING (shop_id = current_shop_id())');
...
DROP POLICY IF EXISTS "Users can view widget configs of their shop" ON widget_configs;
