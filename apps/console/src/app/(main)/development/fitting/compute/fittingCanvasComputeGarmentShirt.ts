...
import { REF_WEIGHT_KG, SH, SIZES } from "../lib/constants";
import { buildTopPlacement } from "../lib/garmentBase";
import { SHIRT_LEFT, SHIRT_RIGHT } from "../lib/pathData";
import { shirtLandmarks } from "../lib/shirtConfig";
import { buildShirtPath } from "../lib/shirtUtils";
import type { MeasureOverlayData, ShirtSize, ShoulderDebug } from "../lib/types";
import { getAllPathPoints } from "../lib/fittingContourUtils";
import { interpolatePath } from "../lib/pathUtils";
import {
  indexOfClosest,
...
export function computeShirtGarmentBranch(input: {
  ...
  if (fromSize && toSize && animProgress < 1) {
    ...
    shirtPathD = interpolatePath(pathFrom, pathTo, smoothStep(animProgress));
  } else {
