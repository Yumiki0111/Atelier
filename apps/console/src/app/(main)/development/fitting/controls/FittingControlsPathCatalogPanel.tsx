  ...
  return (
    <DevPanelSection title="連結頂点 #（採寸）">
      <p className="text-[9px] leading-snug text-muted-foreground">
        ...
        <kbd className="rounded bg-muted px-0.5 font-mono">r</kbd> で列に足せます。
      </p>

      {showMeasureVertexControls ? (
        <div
          className="mt-3 space-y-2 rounded-lg border border-border bg-muted/50 px-2.5 py-2"
          onFocusCapture={() => {
        ...
        >
          <div className="space-y-1.5 rounded-md border border-red-200/80 bg-red-50/50 px-2 py-1.5">
            ...
            <label className="flex flex-col gap-0.5 text-[9px] text-muted-foreground">
              <span>赤線の折れ線</span>
              <input
                className="rounded-md border border-input bg-white px-2 py-1 font-mono text-[10px] outline-none focus:ring-2 focus:ring-red-300/50"
            ...
            {hoveredGarmentVertexIndex != null && Number.isFinite(hoveredGarmentVertexIndex) ? (
              <p className="font-mono text-[9px] text-muted-foreground">
                ホバー中: #{hoveredGarmentVertexIndex}（<kbd className="rounded bg-muted px-0.5">r</kbd> で追加）
              </p>
            ) : null}
            {(() => {
            ...
            <label className="flex flex-col gap-0.5 text-[9px] text-muted-foreground">
              <span className="font-medium text-foreground">長さを変える1本の辺（隣り合う2つの #）</span>
            ...
            {(() => {
              ...
              return t && t[0] !== t[1] ? (
                ...
                <p className="text-[9px] text-destructive">形式を確認（8-15）</p>
              ) : null;
            })()}
            <label className="flex flex-col gap-0.5 text-[9px] text-muted-foreground">
              <span className="font-medium text-foreground">下袖の # 範囲（任意）</span>
              <span className="leading-snug text-muted-foreground">
          ...
          <div className="space-y-1.5 rounded-md border border-primary/35 bg-primary/10 px-2 py-1.5">
            <p className="text-[9px] font-semibold text-primary">反対側の袖（ミラー）</p>
            <p className="text-[9px] leading-snug text-muted-foreground">
            ...
            <label className="flex flex-col gap-0.5 text-[9px] text-muted-foreground">
              <span>袖丈（赤線）</span>
              <input
                className="rounded-md border border-input bg-white px-2 py-1 font-mono text-[10px] outline-none focus:ring-2 focus:ring-primary/45"
            ...
            {(() => {
              ...
              return t && t[0] !== t[1] ? (
                ...
                <p className="text-[9px] text-destructive">形式を確認</p>
              ) : null;
            })()}
            <label className="flex flex-col gap-0.5 text-[9px] text-muted-foreground">
              <span className="font-medium text-foreground">長さを変える1本の辺（隣り合う2つの #）</span>
              <span className="leading-snug text-muted-foreground">例 <strong>93-94</strong>。空欄は自動。</span>
              <input
                className="rounded-md border border-input bg-white px-2 py-1 font-mono text-[10px] outline-none focus:ring-2 focus:ring-primary/45"
            ...
            <label className="flex flex-col gap-0.5 text-[9px] text-muted-foreground">
              <span className="font-medium text-foreground">下袖の # 範囲（任意）</span>
              <span className="leading-snug text-muted-foreground">左の下袖と同じ意味。空欄のときは左の値やガイド推定に従います。</span>
              <input
                className="rounded-md border border-input bg-white px-2 py-1 font-mono text-[10px] outline-none focus:ring-2 focus:ring-primary/45"
          ...
          <div className="space-y-1.5 rounded-md border border-violet-200/80 bg-violet-50/50 px-2 py-1.5">
            ...
            <label className="flex flex-col gap-0.5 text-[9px] text-muted-foreground">
              <span>連結 # の区間</span>
              <input
                className="rounded-md border border-input bg-white px-2 py-1 font-mono text-[10px] outline-none focus:ring-2 focus:ring-violet-300/50"
            ...
            {(() => {
              ...
              return t ? (
                ...
                <p className="text-[9px] text-destructive">形式を確認してください</p>
              ) : null;
            })()}
          </div>
        </div>
