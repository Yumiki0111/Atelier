...
import type { CustomGarmentData, GenericVertexPlotHighlight, MeasureOverlayData, ShoulderDebug } from "@/app/(main)/development/fitting/lib/types";
...
export function computeCustomGarmentBranch(
  ...
  /**
   * path 補間中に着丈メッシュ / 袖スナップが常に目標 size で再適用されると、中間フレームの補間が潰れる。
   */
  ...
  const sizeEnteredForDebug =
    customGarmentData.size.length > 0.5 || customGarmentData.size.sleeve > 0.5;
  if (
    isDebugFittingMeasureEnabled() &&
    !animatingCustomSizeBlend &&
    sizeEnteredForDebug &&
    customGarmentData.presetId === "genericSymmetricTop" &&
    customGarmentData.genericSymmetricTop != null
  ) {
    const g = customGarmentData.genericSymmetricTop;
    console.log("[FIT][custom-garment] transform flags", {
      lengthMeshEligible: canApplyLengthMeshGrade,
      lengthMeshApplied: canvasYGradeScale != null,
      lengthMeshSkipReason: canvasYGradeScale != null ? undefined : lengthMeshSkipReason,
      animatingCustomSizeBlend,
      gradingBaselineLengthCm: g.gradingBaselineLengthCm,
      gradingBaselineSleeveCm: g.gradingBaselineSleeveCm,
      fabricWarpApplied: true,
    });
  }
