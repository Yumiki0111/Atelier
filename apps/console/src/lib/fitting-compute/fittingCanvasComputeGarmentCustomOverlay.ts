...
export type CustomGarmentOverlayAssemblyInput = {
  ...
  /** 着丈 Y 再スケール適用前の縦スパン（ワープ直後メッシュ。ラベル「補正前」） */
...
export function assembleCustomGarmentOverlayAndShoulderDebug(
  ...
  /** 紫着丈: リグ＋ワープのみのときはワープ前（線形 place と bodyPxPerCm が一致）。着丈メッシュ後は最終頂点。 */
  ...
  /**
   * 着丈の幾何数値: 紫区間ありは画面上の縦スパン。`computeGradeLengthVerticalScaleParams` 適用後は
   * size.length×bodyPxPerCm に合わせられる。紫なしは shoulderYForLength〜裾。
   */
  let lengthGeomDebug: { px: number; cm: number };
  if (lengthPathLengthDebug != null) {
    lengthGeomDebug = {
      px: lengthPathLengthDebug.px,
      cm: lengthPathLengthDebug.cm,
    };
    lengthMeasuredCm = lengthPathLengthDebug.cm;
  } else {
    ...
    if (sizeMeaningful && (lenDiff > 0.2 || slDiff > 0.2)) {
      ...
      console.info("[FITTING_MEASURE] 入力値と幾何数値がずれています（採寸オーバーレイの定義差の確認用）", {
        ...
        ...(lengthMismatchWithoutMesh
          ? {
              着丈の注意:
                "縦メッシュ未適用時は紫区間÷bodyPxPerCm が入力着丈と一致しないことがある。applyGenericTopLengthMeshIfEligible が走ると一致しやすい",
            }
          : {}),
      });
    }
